// L2 module tests — client fetch interceptor (design.md §4.7, §5.3).
...
// TC-CLI-RPC-01: exposeAllNamespaces off → settings.describe untouched.
...
// TC-CLI-RPC-01b: exposeAllNamespaces on → bridge namespaces merged in.
...
// TC-BND-CLI-02: third party overrides window.fetch → interceptor still wraps.
test('TC-BND-CLI-02 interceptor wraps original fetch when overwritten', async () => {
  ...
  installClient({ window: win, exposeAllNamespaces: true });
  // third party wraps the current (intercepted) fetch — describe still routed.
  const official = { result: { value: { namespaces: [{ ns: 'shell' }] } } };
