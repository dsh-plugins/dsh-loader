// L2 module tests โ€?host bundle load / lifecycle / rollback (design.md ยง4.1, ยง4.8, ยง4.10, ยง5.5).
...
// TC-LOAD-02: dshloader after a reactive consumer โ€?alias still resolves.
test('TC-LOAD-02 reactive consumer resolves alias regardless of order', async () => {
  ...
  // 1. load downstream first โ?PENDING.
  downstream.tryApply();
...
// TC-ROLL-02: disabled patch flag โ?apply is a no-op (simulated via env).
...
// TC-BND-CON-02: webServer removed then restored โ?effects recycle, no duplicate.
